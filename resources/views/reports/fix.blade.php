    ...
    <div class="d-flex justify-content-center py-3">
        <div style="width: 50%">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="issue" aria-describedby="issue"
                       value="{{$report->issue}}" readonly>
            </div>
            <form action="/reports/updatefix/{{$report->id}}" method="POST" id='fixReport'>
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="issue">Fix Description</label>
                    <textarea class="form-control" id="fix" name='fix' rows="3">{{old('issue') ? old('issue'):$report->fix}}</textarea>
                </div>
            </form>
