...
@section('content')
    ...
    <div class="d-flex justify-content-center py-3">
        <div style="width: 50%">
            <form action="/components/update/{{$component->id}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name"
                           ...
                           value="{{old('name') ? old('name'):$component->name}}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="description" class="form-control @error('description') is-invalid @enderror" id="description"
                           ...
                           value="{{old('description') ? old('description'):$component->name}}">
                    @error('description')
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('category') is-invalid @enderror" name="category" id="category">
                        ...
                        <option value="Processing Device"
                            {{old('category') === 'Processing Device' || $component->category == 'Processing Device' ? 'selected':''}}>Processing Device</option>
                        <option value="Memory Devices"
                            {{old('category') === 'Memory Devices' || $component->category == 'Memory Devices' ? 'selected':''}}>Memory Devices</option>
                        <option value="I/O Devices"
                            {{old('category') === 'I/O Devices' || $component->category == 'I/O Devices' ? 'selected':''}}>I/O Devices</option>
                        <option value="Storage Devices"
                            {{old('category') === 'Storage Devices' || $component->category == 'Storage Devices' ? 'selected':''}}>Storage Devices</option>
                    </select>
                    @error('category')
                    ...
                    <div class="px-5 mt-2">
                        ...
                        <input type="text" name="others" class="form-control" id="others" aria-describedby="others"
                               ...
                               value="{{
                            !($component->category == 'Processing Device'
                            || $component->category == 'Memory Devices'
                            || $component->category == 'I/O Devices'
                            || $component->category == 'Storage Devices') ? $component->category:''
                        }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="model_number" class="form-control @error('model_number') is-invalid @enderror" id="model_number"
                           ...
                           value="{{old('model_number') ? old('model_number'):$component->model_number}}">
                    @error('model_number')
                    ...
                    @if($rooms->count())
                        <select class="form-control @error('room') is-invalid @enderror" name="room" id="room">
                            ...
                            @foreach($rooms as $room)
                                <option value="{{$room->id}}" {{old('room') === $room->id || $component->room->id === $room->id ? 'selected':''}}>{{$room->name}}
                                </option>
                            @endforeach
                        </select>
                    @else
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" name="status" id="status">
                        ...
                        <option value="AVAILABLE"
                            {{old('status') === 'AVAILABLE' || $component->status === 'AVAILABLE' ? 'selected':''}}>AVAILABLE</option>
                        <option value="UNAVAILABLE"
                            {{old('status') === 'UNAVAILABLE' || $component->status === 'UNAVAILABLE' ? 'selected':''}}>UNAVAILABLE</option>
                        <option value="BORROWED"
                            {{old('status') === 'BORROWED' || $component->status === 'BORROWED' ? 'selected':''}}>BORROWED</option>
                    </select>
                    @error('status')
